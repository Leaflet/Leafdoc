...
import {getTemplate, setTemplateDir, setAKAs, engine} from './template';
import * as regexps from './regexps';
import parserTrivial from './parsers/trivial';
import parserCLike from './parsers/c-like';
...
export default class Leafdoc {
	...
	_documentableLabels: Record<string, string> = {
	...
	*/
	...
	registerDocumentable (name: string, label: string, inheritable?: boolean) {
	...
	setLeadingCharacter (char: string) {
	...
	// instead of source.
	addDir (dirname: string, extensions?: string[]) {
	...
	addFile (filename: string, isSource: boolean) {
	...
	addBuffer (buf: Buffer, isSource: boolean) {
	...
	addStr (str: string, isSource: boolean) {
			...
			for (let i in directives) {
				...
				if (scope === 'dc') {
					...
					if (this._knownDocumentables.indexOf(directive) !== -1) {
						...
						let name = '', paramString, params: any = {}, type = null, defaultValue = null, optional = false;
	...
	*/
	...
	_stringifyNamespace (namespace: any, isMini?: boolean) {
	...
	_stringifySupersection (supersection: any, ancestors: any, namespacename: any, isMini?: boolean) {
		var sections = '';
		var inheritances = '';
	...
	_stringifySection (section: any, documentableType: any, inheritingNamespace?: boolean) {
	...
	_normalizeName (namespace: any, name?: string) {
		var id = namespace + (name ? '-' + name : '');
		id = id.trim().replace(/[\s\.]/g, '-');
		return id.toLowerCase();
	}

	_assignAKAs (id: string, akas: any) {
		this._AKAs[id] = id;
		for (var i in akas) {
	...
	_flattenInheritances (classname: string, inheritancesSoFar: any[] = []): any {
